...
numbers). Reproduce with `python scripts/run_bench.py` — note the script exits with a
non-zero code because the `datasets` streaming loader segfaults at interpreter teardown (a
known pyarrow/GIL issue) *after* both JSON files are written; verify success by checking
that `bench_results/real_survival.json` and `bench_results/synthetic_cif.json` exist and
are non-empty, not by the exit code.
